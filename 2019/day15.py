...
def determine_movement(movement, output):
    if output == 0:
        if movement == 1:
            return 4
        if movement == 4:
...
def draw_walls(mappings, position, path):
    ...
    start_x = int(2 * max_x / 2)
    start_y = int(2 * max_y / 2)
...
def run1():
    program = [int(s) for s in STRING.split(',')]
    i = IntCodeCPU(program)
    position = complex(0)
    movement = 1
    mappings = set()
    path = {position}
    oxygen = 0
    finish = False
    while not oxygen or not finish:
        i.run([movement])
        output = i.pop_output()[0]
        new_pos = move(position, movement)
        if new_pos == complex(0):
            finish = True

        movement = determine_movement(movement, output)
    ...
    if path & mappings:
        print('fuck')
    # drawing = draw_walls(set(mappings),oxygen, path)
    # [print(''.join(y)) for y in drawing]
    g = nx.Graph()
    g.add_node(complex(0))
    for x in path:
        for y in get_adjacent(x):
            if y in path:
                g.add_edge(x, y)
...
if __name__ == "__main__":
    ...
    # e = run2()
    print(time.time() - a)
    print(f"from start to oxygen", f)
    print(f"Furthest point away", e)
