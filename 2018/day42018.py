...
test_string = '''[1518-11-01 00:00] Guard #10 begins shift
...
string='''[1518-08-06 00:13] falls asleep
...
class ShiftInfo:
    ...
    def minutes_asleep(self):
        if self.fall_times and self.wake_times:
            return sum(j -i for i,j in self.sleep_wake_cycles)
        else:
            return 0

    @property
    def sleep_wake_cycles(self):
        return [(i,j) for i,j in zip(self.fall_times, self.wake_times)]
...
def get_shift_info_from_string(shift_string):
    shift_strings = sorted(shift_string.split('\n'))
    current_date = None
    shifts = []
    shift = None
    for shift_str in shift_strings:
        log = tuple(filter(None, re.split("[\[\]\-: #]", shift_str)))
        y, m, d, hh, mm = tuple(int(log[x]) for x in range(5))
        date_time = datetime.datetime(y, m, d, hh, mm)
        action = Action(log[5])
        if action == Action.start:
            ...
            shift = ShiftInfo(current_date)
            shift.guard = guard

        elif action == Action.sleep:
            shift.fall_times.append(date_time.minute)
        elif action == Action.wake:
            shift.wake_times.append(date_time.minute)
        else:
            print('fuckup')
    shifts.append(shift)
    return shifts
...
def get_sleep_minutes(shifts: [ShiftInfo]):
    sleeps = defaultdict(int)
    for shift in shifts:
        sleeps[shift.guard] += shift.minutes_asleep
    return sleeps


def get_slept_minutes_count_by_guard(shifts: [ShiftInfo]):
    minutes= defaultdict(lambda: defaultdict(int))
    for shift in shifts:
        guard= shift.guard
        for cycle in shift.sleep_wake_cycles:
            start,end = cycle
...
def run1():
    shifts = get_shift_info_from_string(string)
    sleep_minutes_by_guard = get_sleep_minutes(shifts)
    mins = 0
    best_guard = None
    for g, m in sleep_minutes_by_guard.items():
        if m >= mins:
            best_guard = g
            mins = m
    ...
    best_guard_shifts = [shift for shift in shifts if shift.guard == best_guard]
    slept_minutes_for_guard= get_slept_minutes_count_by_guard(best_guard_shifts)
    fav_minute = -1
    highest_amount = 0
    for guard, slept_minutes in slept_minutes_for_guard.items():
        for minute, amount in slept_minutes.items():
            if amount > highest_amount:
                highest_amount = amount
                fav_minute = minute

    return best_guard * fav_minute

def run2():
    shifts = get_shift_info_from_string(string)
    best_guard = -1
    slept_minutes_by_guard = get_slept_minutes_count_by_guard(shifts)
    fav_minute = -1
    highest_amount = 0
    for guard, slept_minutes in slept_minutes_by_guard.items():
        for minute, amount in slept_minutes.items():
            if amount > highest_amount:
                highest_amount = amount
                fav_minute = minute
