...
class Cuboid:
    ...
    def unwrap(self):
        return {(x,y,z) for x in range(self.x_min,self.x_max+1) for y in range(self.y_min,self.y_max+1) for z in range(self.z_min, self.z_max +1)}
    ...
    def __sub__(self, o):
        ...
        if self.y_min < ixn.y_min:
            cuboids.append(self.clone(x_min=ixn.x_min,x_max=ixn.x_max,y_max=ixn.y_min-1))
        if self.y_max > ixn.y_max:
            cuboids.append(self.clone(x_min=ixn.x_min,x_max=ixn.x_max,y_min=ixn.y_max+1))

        if self.z_min < ixn.z_min:
            cuboids.append(self.clone(x_min=ixn.x_min,x_max=ixn.x_max,y_min=ixn.y_min,y_max=ixn.y_max,z_max=ixn.z_min-1))
        if self.z_max > ixn.z_max:
            cuboids.append(self.clone(x_min=ixn.x_min,x_max=ixn.x_max,y_min=ixn.y_min,y_max=ixn.y_max,z_min=ixn.z_max+1))
